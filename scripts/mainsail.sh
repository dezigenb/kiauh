...
function get_remote_mainsail_version() {
  ...
  local tags
  tags=$(curl -s "https://gh.llkk.cc/https://api.github.com/repos/mainsail-crew/mainsail/tags" | grep "name" | cut -d'"' -f4)
  echo "${tags}" | head -1
}
...
function get_mainsail_download_url() {
  ...
  if [[ ${mainsail_install_unstable} == "true" ]]; then
    releases_by_tag="https://gh.llkk.cc/https://api.github.com/repos/mainsail-crew/mainsail/tags"
